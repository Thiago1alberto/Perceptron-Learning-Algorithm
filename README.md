...
**Atividade**: Atividade 4 - Regressão Linear (Perceptron Learning Algorithm)
...
### Questão 02
Considere os testes no módulo `test_02_linear_model.py`, implemente o modelo linear que recebe uma matriz com `m` linhas e `d+1` colunas e retorna um vetor \bold{\hat{y}} a seguir.
$$
\bold{\hat{y}} = f(X) = X \cdot \bold{w}
$$
...
$$
\bold{w}^{[t+1]} = \bold{w}^{[t]} - \lambda \frac{2}{N} (X^T X \bold{w} - X^T\bold{y})
$$

A estratégia `Newton Method` atualiza o peso conforme a equação a seguir:
$$
\bold{w}^{[t+1]} = (1 - \lambda)\bold{w}^{[t]} + \lambda (X^TX)^{-1}X^T\bold{y}
$$
